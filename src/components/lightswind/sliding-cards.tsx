...
const SlidingCards: React.FC<SlidingCardsProps> = ({
  ...
  return (
    ...
    >
      ...
      {cards && cards.map(({ id, icon,quote ,bgClass = "bg-gradient-to-br from-pink-300 to-orange-200" }, index) => (
        // <article
        //   key={id}
  ...
  onClick={() => onCardClick?.(index)}
  className={cn(
    "card absolute inset-4 rounded-xl border border-gray-400 shadow-md cursor-grab transition-transform ease-in-out p-4 flex flex-col items-center justify-center",
    bgClass
  )}
>
  <div className="flex flex-col items-center justify-center gap-3 text-white">
    ...
    {/* ICON */}
    <div className={cn("grid place-content-center", cardSize)}>
      {icon}
    </div>
